...
def generate_arm_schedule(loan_amount, initial_rate, loan_term, fixed_period, rate_adjustments, caps):
    """Generate ARM amortization with rate adjustments - FIXED VERSION"""
    schedule = []
    balance = loan_amount
    ...
    if monthly_rate > 0:
        ...
        monthly_payment = balance / total_months
    
    for month in range(1, total_months + 1):
        current_year = (month - 1) // 12 + 1
        month_in_year = (month - 1) % 12 + 1
        
        # Check if we've moved to a new year - save previous year's data
        if current_year > current_year_tracked and yearly_principal > 0:
            schedule.append({
                'Year': current_year_tracked,
        ...
        # Check if rate should adjust (at the start of the adjustment period)
        if month_in_year == 1 and current_year > fixed_period:
            years_since_fixed = current_year - fixed_period
            if (years_since_fixed - 1) % rate_adjustments['frequency'] == 0:
                ...
                new_rate = max(min(new_rate, initial_rate + caps['lifetime_max']), 
                              max(0, initial_rate - 2.0))
                current_rate = new_rate
                monthly_rate = current_rate / 100 / 12
                
                # Recalculate payment with new rate
        ...
        # If this is the last month of the year or the last month overall, save the year
        if month_in_year == 12 or month == total_months:
            ...
            yearly_principal = 0
            yearly_interest = 0
            current_year_tracked = current_year + 1
